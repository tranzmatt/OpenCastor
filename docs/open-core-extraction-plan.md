...
**Production wiring is out of scope for Phase 1 (in this revised form).** Phase 1 ends with the integration tested in `tests/cert/`. Phase 2 closure (2026-05-04) reframes "production wiring" as **deployment-topology enforcement**, not in-process wiring — the gateway runs as a separate systemd process in front of OpenCastor and owns actuator device files exclusively (cert property GW-001), so envelope-time cert checks are structurally enforced for actuator traffic without `make_app(...)` being embedded inside `castor/api.py`. See `docs/cert-enforcement-architecture.md` for the canonical Phase 2 deliverable.
